...
app.get("/api/download/youtube", async (req, res) => {
  ...
  try {
    ...
    await ytDlp.execPromise([url, "-x", "--audio-format", "mp3", "--audio-quality", "0", "-o", outPath, "--max-filesize", "20m"]);
    res.download(outPath, "youtube_audio.mp3", () => fs.unlinkSync(outPath));
  } catch (e) {
    res.status(500).json({ error: "Gagal download: " + e.message });
  }
});
...
app.get("/api/download/soundcloud", async (req, res) => {
  ...
  try {
    const id = Date.now();
    const outPath = `tmp/${id}.mp3`;
    await ytDlp.execPromise([url, "-x", "--audio-format", "mp3", "-o", outPath, "--max-filesize", "20m"]);
